...
def callback_sub_whole(quotes: dict):
    ...
    if '09:15' <= curr_time <= '09:29':
        ...
        if new_day:
            ...
            sold_names = []
            for name in my_position.keys():
                # 所有持仓天数计数+1
                my_position[name]['held'] += 1
                # TODO: 达到 hold_days 的持仓卖掉

            for sold_name in sold_names:
                del my_position[sold_name]
    ...
    if '09:30' <= curr_time <= '14:56':
        ...
        for symbol in quotes:
            ...
            if (curr_open < last_close * 0.98) and (curr_price > last_close * 1.02):
                if symbol not in my_position.keys():
                    ...
                    logging.info(f"stock: {symbol} open: {curr_open} last_close: {last_close} last_price: {curr_price}")
        ...
        for i in range(buy_count):
            name = selections[i]['symbol']
            price = selections[i]['price']
            buy_volume = math.floor(p.amount_each / price / 100) * 100

            # TODO: 信号出现则以价格买入
            # buy(self.getdatabyname(name), size=buy_volume, price=price, info=name)
            my_position[name] = {
                'held': 0,
                'cost': price,
                'volume': buy_volume,
